...
# Fork| Github | TG : | KarlMarxArchX97 |
...
"
    ...
    start_options[3]="all"
    start_options[4]="setup_archlinux"
...
"
    ...
    curl -s -O --output-dir $HOME https://raw.githubusercontent.com/KarlMarxArchX/termux-setup/main/scripts/archlinux.sh
    sed -i "s/wm_start_cmd/${wm_cmd}/" $HOME/archlinux.sh
    echo -e '\nalias acli="proot-distro login archlinux --termux-home --shared-tmp -- bash" \nalias dgui="bash debian.sh"\n' >> $HOME/.bashrc
    echo '
check_distro(){
    if [[ "$(whoami)" == "root" ]]; then
        export HISTFILE=~/.archlinux_history
    fi
}
check_distro
' >> $HOME/.bashrc
...
setup_apm(){
    ...
    curl -s -O --output-dir $PATH https://raw.githubusercontent.com/KarlMarxArchX/termux-setup/main/bin/apm
    chmod u+x $PATH/apm
...
setup_aria2(){
    ...
    if ! [ -f $PATH/arc ] || $defaults ; then
        curl -s -O --output-dir $PATH https://raw.githubusercontent.com/KarlMarxArchX/termux-setup/main/bin/arc
        chmod u+x $PATH/arc
...
setup_ytdlp(){
    ...
    mkdir -p $HOME/bin
    curl -s -O --output-dir $HOME/bin https://raw.githubusercontent.com/KarlMarxArchX/termux-setup/main/bin/termux-url-opener
    echo -e "${green}Done.${white}"
}

setup_prettify(){
    echo -e "\n4. Downloading and Setting up Prettify script." 
    curl -s -O --output-dir $PATH https://raw.githubusercontent.com/KarlMarxArchX/termux-setup/main/bin/prettify
...
setup_rxfetch(){
    ...
    curl -s -O --output-dir $PATH https://raw.githubusercontent.com/KarlMarxArchX/termux-setup/main/bin/rxfetch
...
change_ui(){
    ...
    curl -s -o $HOME/.termux/colors.properties https://raw.githubusercontent.com/KarlMarxArchX/termux-setup/main/.termux/"${colors}"
    wget -q -O $HOME/.termux/font.ttf https://raw.githubusercontent.com/KarlMarxArchX/termux-setup/main/.termux/MesloLGS_NF_Bold.ttf
...
save_setup_sh(){
    if ! [ -f $PATH/setup-termux ]; then
        ...
        echo -e '#!/bin/bash\nbash -c "$(curl -fsSL https://raw.githubusercontent.com/KarlMarxArchX/termux-setup/main/setup.sh)"' > $PATH/setup-termux
